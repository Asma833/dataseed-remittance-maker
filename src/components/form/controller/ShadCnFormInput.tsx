...
const ShadCnFormInput = forwardRef<HTMLInputElement, ShadCnFormInputProps>(
  ({ className, type = 'text', forcedValue, uppercase, onInputChange, isIndianPhone, ...props }, ref) => {
    ...
    const inputProps = {
      ...
      className: cn(
        'form-input flex h-9 w-full border border-input bg-transparent px-3 py-1 text-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-80 truncate',
