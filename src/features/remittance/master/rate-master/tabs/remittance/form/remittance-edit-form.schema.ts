...
export const remittanceEditFormSchema = z.object({
  marginType: z.enum(['number', 'percentage']).refine((val) => val, 'Margin type is required'),
  ...
  'ttMargin10-12': z.union([z.number(), z.string()]).refine((val) => {
  ...
  'ttMargin12-02': z.union([z.number(), z.string()]).refine((val) => {
    if (typeof val === 'string') {
      const num = parseFloat(val);
      return !isNaN(num) && num >= 0;
    }
  ...
  'ttMargin02-3-30': z.union([z.number(), z.string()]).refine((val) => {
    if (typeof val === 'string') {
      const num = parseFloat(val);
      return !isNaN(num) && num >= 0;
    }
  ...
  'ttMargin03-30end': z.union([z.number(), z.string()]).refine((val) => {
    if (typeof val === 'string') {
      const num = parseFloat(val);
      return !isNaN(num) && num >= 0;
    }
  ...
  ttHolidayMargin: z.union([z.number(), z.string()]).refine((val) => {
    if (typeof val === 'string') {
      const num = parseFloat(val);
      return !isNaN(num) && num >= 0;
    }
  ...
  ttWeekendMargin: z.union([z.number(), z.string()]).refine((val) => {
    if (typeof val === 'string') {
      const num = parseFloat(val);
      return !isNaN(num) && num >= 0;
    }
