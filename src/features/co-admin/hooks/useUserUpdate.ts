      ...
      mutationFn: async ({ data, productOptions, id }: any) => {
        ...
        const payload = {
          ...
          product_ids: Object.keys(data.productType)
            ...
            .map((key) => productOptions?.find((product: any) => product.name.toLowerCase() === key.toLowerCase())?.id) // Find ID dynamically
            .filter((id) => id),
          role_id: data.roleId,
          is_active: data.isActive,
        };
  
        return await userApi.userUpdate(payload); // Call API with prepared payload
      },
      onSuccess: () => {
