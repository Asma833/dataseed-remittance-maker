...
export const beneficiaryDetailsSchema = z.object({
  ...
  beneficiary_account_number_iban_number: z.string()
  ...
  beneficiary_bank_address: z.string()
  ...
  nostro_charges: z.string()
    ...
    .regex(alphanumericRegex, 'Only alphanumeric characters allowed, no spaces or hyphens'),
  message_to_beneficiary_additional_information: z.string()
    .min(1, 'Message to beneficiary / additional information is required')
    .regex(nameRegex, 'Message cannot start with space or hyphen'),
  student_name: z.string().optional().refine(
    (val) => !val || val.match(nameRegex),
    'Student name cannot start with space or hyphen'
  ),
  student_passport_number: z.string().optional().refine(
    (val) => !val || val.match(alphanumericRegex),
    'Only alphanumeric characters allowed, no spaces or hyphens'
  ),
  payment_instruction_number: z.string().optional().refine(
    (val) => !val || val.match(alphanumericRegex),
    'Only alphanumeric characters allowed, no spaces or hyphens'
  ),
  university_name: z.string().optional().refine(
    (val) => !val || val.match(nameRegex),
    'University name cannot start with space or hyphen'
  ),
  intermediaryBankDetails: z.enum(['yes', 'no']),
  intermediary_bank_account_number: z.string().optional().refine(
    (val) => !val || val.match(alphanumericRegex),
    'Only alphanumeric characters allowed, no spaces or hyphens'
  ),
  intermediary_bank_swift_code: z.string().optional().refine(
    (val) => !val || val.match(alphanumericRegex),
    'Only alphanumeric characters allowed, no spaces or hyphens'
  ),
  intermediary_bank_name: z.string().optional().refine(
    (val) => !val || val.match(nameRegex),
    'Intermediary bank name cannot start with space or hyphen'
  ),
  intermediary_bank_address: z.string().optional().refine(
    (val) => !val || val.match(nameRegex),
    'Intermediary bank address cannot start with space or hyphen'
  ),
}).refine(
  (data) => {
    if (data.intermediaryBankDetails === 'yes') {
      return !!data.intermediary_bank_account_number &&
  ...
  (data) => data.intermediaryBankDetails !== 'yes' || !!data.intermediary_bank_swift_code,
  { message: 'Intermediary Bank Swift Code is required', path: ['intermediary_bank_swift_code'] }
).refine(
  (data) => data.intermediaryBankDetails !== 'yes' || !!data.intermediary_bank_name,
  { message: 'Intermediary Bank Name is required', path: ['intermediary_bank_name'] }
).refine(
  (data) => data.intermediaryBankDetails !== 'yes' || !!data.intermediary_bank_address,
  { message: 'Intermediary Bank Address is required', path: ['intermediary_bank_address'] }
);
